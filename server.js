...
function runYtdlp(args, cwd) {
  return new Promise((resolve, reject) => {
    ...
    p.on("close", code => {
      ...
      else reject(new Error(stderr.slice(-4000) || `yt-dlp exited with ${code}`));
    });
  });
}

app.get("/api/health", (_req, res) => {
  res.json({ ok: true, service: "social-video-downloader" });
});

app.post("/api/info", async (req, res) => {
  const url = req.body?.url?.trim();
  if (!validUrl(url)) return res.status(400).json({ error: "Valid HTTP/HTTPS URL डालें।" });
  ...
  try {
    ...
    res.status(422).json({
      error: "इस URL की जानकारी प्राप्त नहीं हो सकी। URL सार्वजनिक और वैध होना चाहिए।",
...
app.get("/api/download", async (req, res) => {
  ...
  const work = await mkdtemp(path.join(tmpdir(), "svd-"));
  let finished = false;

  try {
    const output = path.join(work, "%(title).80s-%(id)s.%(ext)s");
    const args = [
      "--no-playlist",
      "--no-warnings",
      "--restrict-filenames",
    ...
    const file = names.find(n => !n.endsWith(".part") && !n.endsWith(".ytdl"));
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="${file.replace(/["\\]/g, "_")}"`
    );
...
app.listen(PORT, () => {
  console.log(`Social Video Downloader API running on port ${PORT}`);
});
